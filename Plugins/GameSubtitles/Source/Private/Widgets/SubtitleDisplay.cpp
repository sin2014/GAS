...
// 释放 Slate 控件并解除格式变更委托，避免子系统继续通知已释放的 UWidget。
...
// 读取 GameInstance 字幕格式、订阅变更事件并创建手动或全局字幕模式的 Slate 控件。
...
// Slate 控件存在时接受新的显示格式并立即重建文本与背景样式。
...
// 将格式枚举映射为资产中的字号、颜色、阴影或描边及背景透明度，并更新 Slate 控件。
...
// 编辑器编译 Widget 时要求配置 SubtitleDisplayOptions 资产，缺失则产生编译错误。
