...
namespace
{
	FPredictionKey GetLatestActivationPredictionKey(const FGameplayAbilitySpec& AbilitySpec)
	{
		const TArray<UGameplayAbility*> AbilityInstances = AbilitySpec.GetAbilityInstances();
		return AbilityInstances.IsEmpty()
			? FPredictionKey()
			: AbilityInstances.Last()->GetCurrentActivationInfo().GetActivationPredictionKey();
	}
}
...
void UAuraAbilitySystemComponent::AbilityInputTagPressed(const FGameplayTag& InputTag)
{
	...
	for (FGameplayAbilitySpec& AbilitySpec : GetActivatableAbilities())
	{
		if (AbilitySpec.GetDynamicSpecSourceTags().HasTagExact(InputTag))
		{
			...
			{
				InvokeReplicatedEvent(EAbilityGenericReplicatedEvent::InputPressed, AbilitySpec.Handle, GetLatestActivationPredictionKey(AbilitySpec));
			}
		}
	}
}
...
void UAuraAbilitySystemComponent::AbilityInputTagReleased(const FGameplayTag& InputTag)
{
	...
	for (FGameplayAbilitySpec& AbilitySpec : GetActivatableAbilities())
	{
		if (AbilitySpec.GetDynamicSpecSourceTags().HasTagExact(InputTag) && AbilitySpec.IsActive())
		{
			...
			InvokeReplicatedEvent(EAbilityGenericReplicatedEvent::InputReleased, AbilitySpec.Handle, GetLatestActivationPredictionKey(AbilitySpec));
		}
	}
}
