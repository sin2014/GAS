...
namespace LyraGameplayTags
{
	...
	// 这些原生标签在 GetMovementModeTagMap() 中与 CharacterMovement 的 MovementMode 枚举对应。
	...
	// 扩展自定义移动模式时必须同时更新 GetCustomMovementModeTagMap() 的标签映射。
	...
	// Unreal 内置 MovementMode 到 GameplayTag 的映射。
	// Unreal Movement Modes
	// 将引擎标准 CharacterMovement 模式转换为对应原生 GameplayTag。
	...
	// 项目自定义 MovementMode 到 GameplayTag 的映射。
	// Custom Movement Modes
	// 保存项目自定义移动模式到 GameplayTag 的扩展映射，当前默认不包含条目。
	...
	// 先请求完全匹配标签；允许部分匹配时遍历全部标签并返回首个名称包含输入文本的项。
