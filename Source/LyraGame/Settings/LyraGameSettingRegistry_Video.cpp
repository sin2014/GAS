...
// 根据平台专用 FramePacingMode 判断仅保留或禁用哪些帧率节奏相关设置。
...
// 记录设置项要求的帧率节奏模式及正向或反向匹配策略。
	...
	// 比较当前平台帧率节奏模式，并从设置界面移除不满足匹配策略的项目。
...
public:
	...
	// 帧率节奏模式不匹配或当前 RHI 不支持动态分辨率时，从设置界面移除该项目。
	virtual void GatherEditState(const ULocalPlayer* InLocalPlayer, FGameSettingEditableState& InOutEditState) const override
	{
		const ELyraFramePacingMode ActualMode = ULyraPlatformSpecificRenderingSettings::Get()->FramePacingMode;
...
// 根据平台的 bSupportsGranularVideoQualitySettings 决定是否提供各独立画质通道设置。
...
public:
	...
	{
		// TODO：当前每次独立画质项变化都会立即应用可伸缩性设置，尚未延迟到统一“应用”操作。
...
// 按平台能力构建显示与画质设置页面，并附加相应编辑条件和依赖关系。
UGameSettingCollection* ULyraGameSettingRegistry::InitializeVideoSettings(ULyraLocalPlayer* InLocalPlayer)
{
...
#if PLATFORM_WINDOWS
		// Windows 用户可通过 Alt+Enter 在设置界面之外改变窗口模式并间接改变分辨率，因此监听视口变化并刷新相关设置项。
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
...
#if PLATFORM_USES_DYNAMIC_HDR_SETTING
		{
			...
			Setting->AddEditCondition(MakeShared<FWhenCondition>(
				...
				{
					// TODO：应查询游戏视口的实际 HDR 状态，并在状态变化时调用 RefreshEditableState 更新校准项可用性。
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			// TODO：为移动端补充 3D 渲染分辨率设置支持。
			//@TODO: Add support for 3d res on mobile

			// 此独立画质项变化后，整体画质预设可能需更新为“自定义”或重新匹配到某个预设。
...
// 按平台帧率节奏模式向视频页面添加帧率限制、动态分辨率及移动端 FPS 选项。
void ULyraGameSettingRegistry::InitializeVideoSettings_FrameRates(UGameSettingCollection* Screen, ULyraLocalPlayer* InLocalPlayer)
{
	//----------------------------------------------------------------------------------
	{
		...
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(ELyraFramePacingMode::DesktopStyle));
		// TODO：无电池设备应隐藏此项，但当前没有可用于判断电池存在性的 API。
